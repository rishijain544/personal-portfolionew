  ...
  app.post("/api/exam-prep", async (req, res) => {
    try {
      ...
      if (!apiKey || apiKey === "MY_GEMINI_API_KEY") {
        // Return structured AI simulation response if key not configured
        return res.json({
          status: "simulated",
...
"summary": string`;
  ...
  app.post("/api/intellibank/score", (req, res) => {
    ...
    // Anomaly & Fraud Probability calculation based on FinTech ML factors
    let baseRisk = 2.5;
    ...
    // Creditworthiness underwriting calculation
